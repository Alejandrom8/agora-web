        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12 }}>
            <TextField
              ...
              value={g.title}
              onChange={(e) =>
                setForm((s) => ({ ...s, general: { ...s.general, title: e.target.value } }))
              }
              fullWidth
              required
              InputProps={{
          ...
          <Grid size={{ xs: 12 }}>
            <TextField
              ...
              value={g.description}
              onChange={(e) =>
                setForm((s) => ({ ...s, general: { ...s.general, description: e.target.value } }))
              }
              fullWidth
              multiline
              minRows={3}
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <TextField
              ...
              value={g.date}
              onChange={(e) =>
                setForm((s) => ({ ...s, general: { ...s.general, date: e.target.value } }))
              }
              fullWidth
              required
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <TextField
              ...
              value={g.coverUrl ?? ''}
              onChange={(e) =>
                setForm((s) => ({ ...s, general: { ...s.general, coverUrl: e.target.value } }))
              }
              fullWidth
              placeholder="https://..."
              InputProps={{
...
const StepAgenda: React.FC<{
  ...
  const remove = (id: string) =>
    setForm((s) => ({ ...s, agenda: s.agenda.filter((e) => e.id !== id) }));
  ...
  return (
    <Stack spacing={2}>
            ...
            <Grid container spacing={2} alignItems="center">
              ...
              <Grid size={{ xs: 12, md: 2 }}>
                <TextField
                  label="Inicio"
                  value={e.startTime}
                  onChange={(ev) => update(e.id, { startTime: formatTimeInput(ev.target.value) })}
                  fullWidth
                  placeholder="09:00"
                />
              </Grid>
              <Grid size={{ xs: 12, md: 2 }}>
                <TextField
                  label="Fin"
                  value={e.endTime}
                  onChange={(ev) => update(e.id, { endTime: formatTimeInput(ev.target.value) })}
                  fullWidth
                  placeholder="10:00"
                />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  label="Portada (URL)"
                  value={e.coverUrl ?? ''}
                  onChange={(ev) => update(e.id, { coverUrl: ev.target.value })}
                  fullWidth
                />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  label="Speaker (opcional)"
                  value={e.speaker ?? ''}
                  onChange={(ev) => update(e.id, { speaker: ev.target.value })}
                  fullWidth
                />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <FormControlLabel
                  control={
                    <Switch
                      checked={e.requiresRegistration}
                      onChange={(ev) => update(e.id, { requiresRegistration: ev.target.checked })}
                    />
                  }
              ...
              <Grid size={{ xs: 12, md: 3 }} sx={{ textAlign: { xs: 'left', md: 'right' } }}>
                <Tooltip title="Eliminar sub-evento">
                  <IconButton color="error" onClick={() => remove(e.id)}>
                    <DeleteRounded />
                  </IconButton>
                </Tooltip>
              </Grid>
      ...
      <Button onClick={add} variant="outlined" startIcon={<AddRounded />}>
        Agregar sub-evento
      </Button>
    </Stack>
  );
...
const StepRoles: React.FC<{
  ...
  const toggle = (role: RoleKey, key: keyof RoleRule, value: never) =>
    setForm((s) => ({
      ...s,
      roles: s.roles.map((r) => (r.role === role ? { ...r, [key]: value } : r)),
    }));
            ...
            <CardContent>
              <Stack spacing={1.25}>
                ...
                <FormControlLabel
                  ...
                  control={
                    <Switch
                      checked={r.mfaRequired}
                      onChange={(e) => toggle(r.role, 'mfaRequired', e.target.checked as never)}
                    />
                  }
                />
                <FormControlLabel
                  ...
                  control={
                    <Switch
                      checked={r.validationRequired}
                      onChange={(e) =>
                        toggle(r.role, 'validationRequired', e.target.checked as never)
                      }
                    />
                  }
                />
                <TextField
                  ...
                  value={r.notes ?? ''}
                  onChange={(e) => toggle(r.role, 'notes', e.target.value as never)}
                  fullWidth
                  multiline
                  minRows={2}
                />
...
const StepCategories: React.FC<{
  ...
  const remove = (c: string) =>
    setForm((s) => ({ ...s, categories: s.categories.filter((x) => x !== c) }));
        ...
        <Stack spacing={2}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
            <TextField
              ...
              fullWidth
            />
            <Button onClick={add} variant="contained" startIcon={<AddRounded />}>
              Agregar
            </Button>
          </Stack>
...
const StepAttendees: React.FC<{
  ...
  const addManual = () =>
    setForm((s) => ({
      ...s,
      attendees: [...s.attendees, { id: uid(), name: '', email: '', role: 'attendee' }],
    }));
  const update = (id: string, patch: Partial<Attendee>) =>
    setForm((s) => ({
      ...s,
      attendees: s.attendees.map((a) => (a.id === id ? { ...a, ...patch } : a)),
    }));
  const remove = (id: string) =>
    setForm((s) => ({ ...s, attendees: s.attendees.filter((a) => a.id !== id) }));
      ...
      <Card>
        <CardContent>
          ...
          >
            ...
            <Button onClick={addManual} variant="contained" startIcon={<AddRounded />}>
              Agregar asistente
            </Button>
          </Stack>
        </CardContent>
        ...
        <Stack spacing={4}>
          ...
          >
            <Box>
              ...
              <Typography color="text.secondary">
                Crea tu evento y configura agenda, roles, categorías y asistentes.
              </Typography>
            </Box>
          </Stack>
          ...
          >
            <Stack direction="row" spacing={1}>
              <Button variant="outlined" color="inherit" disabled={active === 0} onClick={prev}>
                Atrás
              </Button>
