...
const KpiCard: React.FC<{ label: string; value: string | number; icon?: React.ReactNode; helper?: string; color?: "default" | "primary" | "info" | "success" | "warning" | "error"; }> = ({ label, value, icon, helper, color = "default" }) => (
    <Card>
        <CardContent>
            ...
            {helper && (
                <Chip size="small" label={helper} color={color === "default" ? undefined : color} sx={{ mt: 1, width: "fit-content" }} />
            )}
        </CardContent>
    </Card>
);

const CardSection: React.FC<{ title: string; subtitle?: string; action?: React.ReactNode; children: React.ReactNode; }> = ({ title, subtitle, action, children }) => (
    <Card>
        <CardContent>
        ...
        <Stack spacing={3}>
            ...
            <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" alignItems={{ xs: "flex-start", md: "center" }} spacing={2}>
                <Box>
                    ...
                    <Typography color="text.secondary">Panel analítico de tus eventos y asistentes.</Typography>
                </Box>
            ...
            <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                    <KpiCard label="Asistentes totales" value={kpis.totalAttendees} icon={<PeopleAltRounded color="info" />} helper={`+${kpis.growth7d}% vs 7d`} color="success" />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                    <KpiCard label="Founders" value={kpis.totalFounders} icon={<TrendingUpRounded color="info" />} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                    <KpiCard label="Inversionistas" value={kpis.totalInvestors} icon={<ShieldRounded color="info" />} helper={kpis.mfaRequired ? "MFA activo" : undefined} color={kpis.mfaRequired ? "warning" : "default"} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                    <KpiCard label="Sub-eventos" value={kpis.totalSessions} icon={<EventRounded color="info" />} />
                </Grid>
            </Grid>
                ...
                <Grid size={{ xs: 12, md: 7 }}>
                    <CardSection title="Registros por día" subtitle="Últimos 14 días">
                        <Box sx={{ width: "100%", height: 300 }}>
                            <ResponsiveContainer>
                                <LineChart data={seriesRegistrations} margin={{ left: 8, right: 8, top: 10, bottom: 0 }}>
                                    ...
                                    <Tooltip contentStyle={{ background: theme.palette.background.paper, border: `1px solid ${alpha("#FFFFFF", 0.12)}` }} />
                                    <Line type="monotone" dataKey="count" stroke={theme.palette.info.main} strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        </Box>
                    </CardSection>
            ...
            <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 7 }}>
                    <CardSection title="Asistencia por sub-evento" subtitle="Top 5 sesiones" action={
                        <Button size="small" variant="outlined" startIcon={<DownloadRounded />} onClick={() => exportCSV(subEventsDistribution, "subevent_distribution.csv")}>Exportar CSV</Button>
                    }>
                        <Stack direction={{ xs: "column", md: "row" }} gap={2} alignItems={{ xs: "center", md: "stretch" }}>
                            <Box sx={{ flex: 1, height: 280 }}>
                                <ResponsiveContainer>
                                    <PieChart>
                                        <Pie data={subEventsDistribution} dataKey="value" nameKey="name" outerRadius={100} innerRadius={60} paddingAngle={4}>
                            ...
                            <Divider flexItem orientation="vertical" sx={{ display: { xs: "none", md: "block" }, opacity: 0.12 }} />
                            <Box sx={{ flex: 1 }}>
                                <Stack spacing={1.25}>
                                    {subEventsDistribution.map((s, i) => (
                                        <Stack key={s.name} direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 1, borderRadius: 1, backgroundColor: alpha("#FFFFFF", 0.03) }}>
                                            <Stack direction="row" alignItems="center" gap={1.25}>
                                                <Box sx={{ width: 10, height: 10, borderRadius: 0.5, backgroundColor: COLORS[i % COLORS.length] }} />
                ...
                <Grid size={{ xs: 12, md: 5 }}>
                    <CardSection title="Recomendaciones" subtitle="Acciones sugeridas">
                        <Stack spacing={1.25}>
                            <Tip title="Activa MFA para inversionistas" body="Requisito en 1 rol. Mejora la seguridad del acceso a sub-eventos de alto impacto." />
                            <Tip title="Invita a tus speakers por lote" body="Carga un CSV con tus ponentes y automatiza los correos de invitación." />
                            <Tip title="Habilita registro por sesión" body="Permite medir demanda y controlar aforo por taller o pitch stage." />
                        </Stack>
                    </CardSection>
                </Grid>
            </Grid>
...
const Tip: React.FC<{ title: string; body: string }> = ({ title, body }) => (
    <Box sx={{ p: 1.25, borderRadius: 1.5, backgroundColor: (t) => alpha("#FFFFFF", 0.04), border: (t) => `1px solid ${alpha("#FFFFFF", 0.08)}` }}>
